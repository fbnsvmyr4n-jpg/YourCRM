...
describe("aiming the sun and moon at the frame", () => {
   ...
   */
  /* The scene's own camera: 58° vertical, pitched 41° down, on a 2194x1726
     drawing buffer. */
  const CAMERA = {
  ...
  it("keeps a body that has set behind the limb, not pinned to it", () => {
    ...
    const set = (Math.asin(aimBody(-3, 0, CAMERA).direction[2]) * 180) / Math.PI;
  ...
  it("brings the sun into frame at the times it must be seen", () => {
    ...
    const aim = aimBody(2.67, 271.7, CAMERA);
    const azimuth = (Math.atan2(aim.direction[0], aim.direction[1]) * 180) / Math.PI;
    expect(Math.abs(azimuth)).toBeLessThan(60);
  });

  it("does not flatten a body that is nowhere near the limb", () => {
    expect(aimBody(40, 0, CAMERA).limbProximity).toBe(0);
    expect(aimBody(0, 0, CAMERA).limbProximity).toBe(1);
  });

  it("survives a body whose position could not be computed", () => {
    const aim = aimBody(Number.NaN, 120, CAMERA);

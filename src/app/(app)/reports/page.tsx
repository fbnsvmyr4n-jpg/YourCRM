  ...
  return (
    <div className="mx-auto max-w-[1500px] animate-fade-up">
      ...
      {/* Progress against the monthly target.
          ...
          Above the tabs and outside them, because it is the one figure that
          says whether the rest is good news. Revenue Won answers "how much";
          this answers "is that enough", and the second question is the reason
          anybody opens a reports page. */}
      <div className="mt-5">
        <SalesTargetCard
          pct={targetPct}
          won={wonThisMonth}
      ...
      {/*
          Grouped by the page each figure is about, and named the way the
          sidebar names it.

          It was eleven cards in no order anybody could state — revenue, leads,
          voice, lead status, meetings, deals again — so finding "how are my
          leads doing" meant scrolling past everything else and recognising the
          right heading. Only sections with something in them are offered: a tab
        ...
        sections={[
          {
            ...
            hint: `${money(r.revenueWon)} won · ${money(r.openPipeline)} open`,
            content: (
              <>
                <div className="grid grid-cols-1 gap-5 @min-[880px]:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
                  <Card>
                    <CardHeader
                      title="Revenue won"
          ...
          },
          {
            id: "leads",
            label: "Leads",
            tone: { color: "var(--purple)", soft: "var(--purple-soft)" },
            hint: `${rate(r.leadConversion)} converted`,
            content: (
              <div className="grid grid-cols-1 gap-5 @min-[880px]:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
                <Card>
                  <CardHeader
          ...
          },
          {
            ...
            hint: `${r.meetings.total} booked · ${rate(r.meetings.showRate)} show rate`,
            content: (
              <>
          ...
          ...(accounts.length > 0 || referrers.length > 0
            ? [
                {
                  id: "companies",
                  label: "Companies",
                  tone: { color: "var(--amber)", soft: "var(--amber-soft)" },
                  hint: `${accounts.length} with deals`,
                  content: (
                    <>
                      {accounts.length > 0 && (
                        <Card>
            ...
            : []),
          {
            id: "voice",
            label: "Voice",
            tone: { color: "var(--red)", soft: "var(--red-soft)" },
            hint: `${r.voice.calls} call${r.voice.calls === 1 ? "" : "s"}`,
            content: (
              <Card className="flex flex-1 flex-col">
                <CardHeader
                  title="Voice agent"
